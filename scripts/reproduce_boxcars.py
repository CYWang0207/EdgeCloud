#!/usr/bin/env python3
"""Reproduction entrypoint for the fixed BoxCars116k Adapter evaluation."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "artifacts/boxcars/reproduced")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--dry-run", action="store_true")
    args, extra = parser.parse_known_args()
    command = [sys.executable, str(ROOT / "module_edge_perception/evaluate_boxcars_cloud_teacher_quick_test.py"),
               "--dataset-path", str(args.dataset_path), "--baseline-checkpoint", str(args.baseline_checkpoint),
               "--adapter-checkpoint", str(args.adapter_checkpoint), "--output-dir", str(args.output_dir),
               "--samples", "0", "--seed", str(args.seed), *extra]
    print(" ".join(command))
    if not args.dry_run:
