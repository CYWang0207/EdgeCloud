...
> **架构更新（2026-08-09）**：本文中关于 `z_vlm -> z_edge`、FiLM 条件 AdaptFormer、VLM condition cache 和 edge-side 环境控制器的内容已不再是正式路线。实验表明它们不能有效改善低层相机退化校正，保留它们仅用于 VLM-conditioned Adapter 消融。当前正式设计、训练代码与接口定义见[云端视觉教师 Adapter 刷新方案](云端视觉教师Adapter方案_20260809.md)。
