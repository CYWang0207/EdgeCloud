...
**口径说明（重要）**：本节 99.8% 量级的业务保持率是**仿真器口径**——`proxy_acc` 采用 Critic 的 `β_actual` 代理精度，且 `u=0` 时 `realtime_comm=0`，并非真模型实测。真模型全链路（`main_edge_cloud_real_model.py`，8/12 clean-guard adapter，acc_floor=0.8）实测业务保持率为 **90.28–90.32%**（四档网络模式），该数才是申报硬指标的数字；本节 99.8% 仅用于说明调度算法相对效果与异步架构的连续性，不得作为硬指标申报数。
