...
def main():
    ...
    # condition_dim=0 is intentional: cloud teacher never enters edge forward.
    attach_adaptformer(model, r=a.r, condition_dim=0)
    ...
    for epoch in range(a.epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with torch.no_grad(), torch.autocast("cuda", dtype=amp):
                clean_z, _ = model(clean, view_mask=mask, return_features=True, apply_quality_gate=False)
            set_adapter_enabled(model, True)
            with torch.autocast("cuda", dtype=amp):
                student_z, student_h = model(corrupt, view_mask=mask, return_features=True, apply_quality_gate=False)
                replay_z = model(clean, view_mask=mask, apply_quality_gate=False)
        ...
        with torch.no_grad():
            for name, loader in val_loaders.items():
                ...
                for step, batch in enumerate(loader):
                    ...
                    with torch.autocast("cuda", dtype=amp):
                        z = model(images, view_mask=mask, apply_quality_gate=False)
